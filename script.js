...
window.loadClassResults = async () => {
  ...
  try {
    ...
    for (const studentDoc of studentsSnap.docs) {
      ...
      resultsSnap.forEach(rDoc => {
        ...
        if (r.name === student.name && r.exam === exam) {
          ...
          studentData.totalPercentage += perc; // sum of percentages
          studentData.count++;
      ...
      studentData.mean = studentData.count > 0 ? Math.round(studentData.totalPercentage / studentData.count) : 0;
    ...
    allStudents.forEach((s, index) => {
      if (s.totalPercentage !== lastTotal) position = index + 1;
      s.position = position;
      lastTotal = s.totalPercentage;
    });
    ...
    document.querySelectorAll('.updateBtn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          // Update row totals
          const mean = count > 0 ? Math.round(totalPerc / count) : 0;
          row.querySelectorAll('td')[subjects.length + 2].innerText = totalPerc;
