...
// ----------------- LOAD & EDIT CLASS RESULTS (AUTO-SAVE) -----------------
window.loadClassResults = async () => {
  ...
  try {
    ...
    for (const studentDoc of studentsSnap.docs) {
      ...
      studentData.mean = Object.keys(studentData.subjects).length > 0 
        ? (studentData.total / Object.keys(studentData.subjects).length).toFixed(2) 
        : "0.00";
    ...
    const recalcTotals = () => {
      ...
      allStudents.forEach(student => {
        ...
        let count = 0;
        subjects.forEach(sub => {
          const val = parseFloat(student.subjects[sub]?.percentage || 0);
          total += val;
          if (!isNaN(val)) {
            count++;
        ...
        student.total = total;
        student.mean = count > 0 ? (total / count).toFixed(2) : "0.00";
        classTotalMean += parseFloat(student.mean);
      });

      const classMean = allStudents.length > 0 ? (classTotalMean / allStudents.length).toFixed(2) : "0.00";
      return classMean;
    };
    ...
    const renderTable = () => {
        ...
        subjects.forEach(sub => {
          ...
          html += `<td>
            <input type="number" min="0" max="100" value="${val}" 
                   style="width:50px" 
                   data-student="${s.name}" data-subject="${sub}">
          </td>`;
        });
      ...
      subjects.forEach(sub => {
        const mean = subjectCounts[sub] > 0 ? (subjectTotals[sub] / subjectCounts[sub]).toFixed(2) : "0.00";
        html += `<td>${mean}</td>`;
      });
      html += `<td>-</td><td>-</td><td>-</td></tr>`;

      // Class mean row
      const classMean = recalcTotals();
      html += `<tr>
...
`;
    ...
    // Attach input auto-save
    const attachInputListeners = () => {
      document.querySelectorAll('input[data-student]').forEach(input => {
        input.addEventListener('input', debounce(async (e) => {
          ...
          if (studentObj) studentObj.subjects[subject] = { percentage: val };

          // Update Firestore
          const r = resultsSnap.docs.find(d => {
            const data = d.data();
            return data.name === studentName && data.exam === exam && data.subject === subject;
          });
...
snap.forEach(doc => {
  const data = doc.data(); // ✅ ADD THIS LINE

  const option = document.createElement("option");
  option.value = data.name;
  option.textContent = `${data.name} (${data.class})`;

  document.getElementById("marksStudentSelect").appendChild(option);
});
...
document.getElementById('marksClassSelect')
  .addEventListener('change', populateStudentsByClass);
